...
app.post("/jpg_to_pdf", multipartyMiddleware, function(req, res) {
  fs.readFile(req.files.file.path, function(err, data) {
    var file = req.files.file;
    file.fullPath =
      config.TEMP + slugify(file.name, { remove: /[*+~()'"!:@]/g });
...
app.post("/convert_to_pdf", jsonParser, function(req, res) {
  ...
  data.forEach(function(element, index, array) {
    element = slugify(element, { remove: /[*+~()'"!:@]/g });
    var file = {};
...
app.post("/convert_word_to_pdf", jsonParser, function(req, res) {
  ...
  data.forEach(function(element, index, array) {
    ...
    fs.rename(
      "/var/www/converter/temp/" + element,
      "/var/www/converter/temp/" + element.replace(/\s/g, ""),
      function(err) {
        if (err) {
          res.status(404);
        ...
        file.writeTo = config.TEMP + file.convertTo;
        var code = shell.exec(
          "sudo mv /var/www/converter/temp/" +
            element +
            " " +
        ...
        var code = shell.exec(
          "sudo /snap/bin/libreoffice -convert-to pdf:writer_pdf_Export '" +
            config.LIBRE_OFFICE_PATH +
            element +
            "'"
        ).code;
        ...
        archive.pipe(output);
        archive.append(
          fs.createReadStream(config.LIBRE_OFFICE_OUTPUT_PATH + file.convertTo),
          {
            name: file.convertTo
          }
        );
...
app.post("/convert_from_pdf_word", jsonParser, function(req, res) {
  ...
  data.forEach(function(element, index, array) {
    ...
    file.writeTo = config.TEMP + file.convertTo;
    fs.rename(element, element.replace(/\s/g, ""), function(err) {
      if (err) {
        res.status(404);
        res.send("Error occured");
        console.log(err);
      ...
      file.writeTo = config.TEMP + file.convertTo;
      var code = shell.exec(element + " " + config.LIBRE_OFFICE_PATH + "").code;
      if (code !== 0) {
        res.status(404);
        res.send("Error occured");
        console.log(err);
      ...
      var code = shell.exec(
        "sudo /snap/bin/libreoffice --infilter='writer_pdf_import' --convert-to doc '" +
          config.LIBRE_OFFICE_PATH +
          element +
          "'"
      ).code;
